...
class EmailTemplateController extends Controller
{
    ...
    public static function getDataByType($type, $data = null)
    {
        try {
            ...
            if (in_array($type, ['new_edited_job_available', 'new_job_available', 'new_plan_purchase', 'new_user_registered', 'plan_purchase', 'new_pending_candidate', 'new_candidate', 'new_company_pending', 'new_company', 'update_company_pass', 'verify_subscription_notification', 'email_verify'])) {
                $return_data = $data;
            }
    ...
    public static function getFormatterByType(string $type, $data = null)
    {
        try {
            ...
            if ($type === 'email_verify') {
                $format['search'] = ['{verify_email}'];
                if ($data !== null) {
                    $format['replace'] = [$data['verify_email']];
                }
            }
