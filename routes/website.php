...
// verify-email
Route::get('/verify-email/{token}', function (Request $request) {
    ...
    if (! session()->has('user_data')) {
        abort(400, 'Session data missing. Please try registering again.');
    }

    $data = session()->get('user_data');

    // 🔥 Create the user & retrieve the instance
    $user = User::create([
        'role' => $data['role'],
        'name' => $data['name'],
    ...
    return $user->role === 'company'
        ? redirect()->route('company.dashboard', ['verified' => true])
        : redirect()->route('candidate.dashboard', ['verified' => true]);
